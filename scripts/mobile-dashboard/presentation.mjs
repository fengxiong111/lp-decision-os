...
:root {
  ...
  --accent: #7257a8;
  --content: 100%;
  --pool-columns: 64px minmax(360px, 2.4fr) minmax(180px, .9fr) minmax(220px, 1fr) minmax(220px, 1fr) minmax(220px, 1fr);
}
...
body {
  ...
  color: var(--ink);
  font: 17px/1.45 -apple-system, BlinkMacSystemFont, "SF Pro Text", "PingFang SC", "Microsoft YaHei", sans-serif;
  font-variant-numeric: tabular-nums;
  -webkit-font-smoothing: antialiased;
}
main { width: calc(100% - 96px); margin: 0 auto; padding: 26px 0 72px; }
...
.pool-table-head { padding: 18px 20px 16px; border-top: 1px solid var(--line); border-bottom: 1px solid var(--line); color: var(--soft); font-size: 13px; font-weight: 680; letter-spacing: .04em; }
.pool-table-head > div:nth-child(-n+3) { text-align: left; }
.pool-table-head > div:nth-child(n+4) { text-align: right; }
.pool-row { padding: 28px 20px; border-bottom: 1px solid rgba(226, 224, 216, .78); }
.pool-row:hover { background: rgba(255, 255, 255, .42); }
.pool-rank { color: var(--soft); font-size: 15px; }
.pool-identity, .pool-venue { min-width: 0; text-align: left; }
.pool-identity strong { display: block; overflow: hidden; font-size: 24px; font-weight: 760; letter-spacing: -.04em; line-height: 1.12; text-overflow: ellipsis; white-space: nowrap; }
.pool-venue strong { display: block; font-size: 18px; font-weight: 700; }
.pool-metric { min-width: 0; text-align: right; }
.pool-metric strong { display: block; overflow: hidden; font-size: 21px; font-weight: 700; white-space: nowrap; text-overflow: ellipsis; }
.pool-fee strong { color: var(--ink); font-size: 24px; font-weight: 800; }
...
@media (min-width: 1101px) and (max-width: 1400px) {
  :root { --pool-columns: 52px minmax(260px, 1.75fr) minmax(140px, .9fr) minmax(170px, 1fr) minmax(170px, 1fr) minmax(170px, 1fr); }
  main { width: calc(100% - 64px); }
  .pool-table-head, .pool-grid { column-gap: 10px; }
  .pool-table-head { padding-left: 14px; padding-right: 14px; }
  .pool-row { padding-left: 14px; padding-right: 14px; }
}
@media (max-width: 720px) {
  ...
  .pool-identity strong { font-size: 19px; }
  .pool-venue strong { font-size: 14px; }
...
export function renderPage({ snapshotHash = null, runtimeVersion = null }) {
  ...
  return `<!doctype html>
<html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#f6f5ef"><meta name="data-source" content="./top3.json"><title>Solana LP Market</title><style>${renderStyles()}</style></head><body><main data-top3-source="./top3.json">
<section class="pool-table" role="table" aria-label="Solana LP fee market"><div class="pool-table-head" role="row"><div role="columnheader">#</div><div role="columnheader">池</div><div role="columnheader">DEX</div><div role="columnheader">TVL</div><div role="columnheader">Volume</div><div role="columnheader">Fee</div></div><div id="pool-list" aria-live="polite"></div><div id="empty-state" class="empty-state" hidden></div></section>
