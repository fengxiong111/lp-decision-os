...
function usd(value, digits = 2) {
  return Number.isFinite(value) ? `$${numberValue(value, digits)}` : "UNAVAILABLE";
}
...
function scoreLabel(value) {
  return Number.isFinite(value) ? numberValue(value, 1) : "UNAVAILABLE";
}
...
export function renderRow(row) {
  ...
  return `<article class="optimizer-row" data-pool-address="${escapeHtml(row.poolAddress ?? "")}" role="row">
    <div class="row-grid">
      ...
      <div class="score" role="cell"><strong>${escapeHtml(scoreLabel(value.opportunityScore))}</strong></div>
      <div class="net" role="cell"><strong>${escapeHtml(usd(value.netEstimate, 2))}</strong></div>
      ...
      <div class="confidence" role="cell"><strong>${escapeHtml(percentageLabel(value.confidence))}</strong></div>
      <div class="action ${escapeHtml(value.action.toLowerCase().replaceAll("_", "-"))}" role="cell"><strong>${escapeHtml(value.action)}</strong></div>
    </div>
...
function renderStyles() {
  return `
:root{--paper:#f5f4ed;--ink:#141413;--muted:#716f68;--soft:#96938b;--line:#dedbd0;--blue:#1b365d;--danger:#8a4e3d;--warn:#8a6b35;--pass:#3e6650;--content:1180px;--columns:minmax(180px,1.4fr) minmax(115px,.75fr) minmax(135px,1fr) minmax(145px,1fr) minmax(145px,1fr) minmax(105px,.7fr) minmax(125px,.8fr)}
...
.table-header,.row-grid{display:grid;grid-template-columns:var(--columns);column-gap:18px;align-items:center}.table-header{position:sticky;top:0;z-index:2;padding:11px 10px 9px;border-top:1px solid var(--line);border-bottom:1px solid var(--line);background:rgba(245,244,237,.96);color:var(--soft);font-size:11px;letter-spacing:.06em}.table-header div:not(:first-child){text-align:right}.optimizer-row{padding:25px 10px 14px;border-bottom:1px solid var(--line)}.row-grid>div:not(:first-child){text-align:right}.pair{min-width:0;text-align:left!important}.pair strong{overflow:hidden;font-family:"Iowan Old Style","Songti SC",STSong,Georgia,serif;font-size:23px;font-weight:750;letter-spacing:-.03em;line-height:1.05;text-overflow:ellipsis;white-space:nowrap}.score,.net,.strategy,.confidence,.action{display:grid;gap:4px;min-width:0}.score strong{color:var(--blue);font-size:20px;font-weight:750;line-height:1.05;white-space:nowrap}.net strong{color:var(--blue);font-size:20px;font-weight:700;line-height:1.05;white-space:nowrap}.strategy strong{font-size:14px;font-weight:650;line-height:1.15;white-space:nowrap}.strategy em{overflow:hidden;color:var(--muted);font-size:12px;font-style:normal;text-overflow:ellipsis;white-space:nowrap}.confidence strong{font-size:14px;font-weight:650;white-space:nowrap}.action strong{color:var(--blue);font-size:13px;font-weight:750;letter-spacing:.04em;white-space:nowrap}.action.blocked strong{color:var(--danger)}.action.review strong{color:var(--warn)}.action.watch strong{color:var(--muted)}.row-tools{display:flex;justify-content:flex-end;margin-top:10px}.why-trigger{border:0;padding:0;background:transparent;color:var(--muted);font:600 11px/1.2 inherit;letter-spacing:.1em;cursor:pointer}.why-trigger:hover{color:var(--blue)}.opportunity-evidence{margin:22px 0 0;padding-top:16px;border-top:1px solid var(--line)}.opportunity-evidence h3{margin:0 0 10px;color:var(--muted);font-size:11px;letter-spacing:.08em}.opportunity-evidence>div{display:flex;justify-content:space-between;align-items:baseline;gap:16px}.opportunity-evidence strong{color:var(--blue);font-size:22px}.opportunity-evidence small{display:block;margin-top:6px;color:var(--muted);font-size:11px}
...
<div class="status-bar" aria-live="polite"><span id="live-status" data-state="warning">读取 top3.json</span><span id="market-count">CANDIDATES 0 / 3</span></div>
<section class="ranking" aria-label="RWA / USDC Top 3 opportunities" role="table"><div class="table-header" role="row"><div role="columnheader">Pair</div><div role="columnheader">Opportunity Score</div><div role="columnheader">Net Estimate</div><div role="columnheader">Core</div><div role="columnheader">Buffer</div><div role="columnheader">Confidence</div><div role="columnheader">Action</div></div><div id="ranking-list" class="ranking-list" aria-live="polite"></div><div id="empty-state" class="empty-state" hidden></div></section>
...
<footer>唯一数据源：mobile-dashboard/top3.json · Top 3 机会层 + Verification 证据层 · <span id="observed-wrap"${initialTimestamp ? "" : " hidden"}>读取：<span id="observed-at">${escapeHtml(initialTimestamp ?? "")}</span></span><a href="https://github.com/fengxiong111/lp-decision-os">查看源码</a></footer></main><script type="module" src="./runtime.js${runtimeSrc}"></script></body></html>`;
}
