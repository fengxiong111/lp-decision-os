...
function snapshotWindowStatus(snapshot: DashboardSnapshot, key: TerminalWindow): RankingWindowStatus {
  if (key === "24h") {
    ...
    return { key, label: "24小时", status: available ? "COMPLETE" : "UNAVAILABLE", state: available ? "官方API" : "不可用", enabled: available, progress: available ? 100 : null, coverage: available ? 1 : null, completedPools: available ? snapshot.pools.length : null, targetPools: snapshot.pools.length, universeLabel: available ? `官方 API · ${snapshot.pools.length} 个 Pool` : "官方 API 数据不可用", updatedAt: snapshot.generatedAt, source: "Raydium 官方 API v3 · 24h as_of", reason: available ? null : "官方24h数据不可用" };
  }
  ...
  const status: RankingWindowStatus["status"] = (coverage.backfillStatus === "COMPLETE" || coverage.backfillStatus === "LIVE") && ratio === 1 && coverage.gapSlots === 0 && coverage.unknownInstructions === 0
    ? "COMPLETE"
...
function WindowButton({ view, active, onClick }: { view: RankingWindowStatus; active: boolean; onClick: () => void }) {
  const suffix = view.key === "24h" && view.status === "COMPLETE"
    ...
    : view.completedPools !== null && view.targetPools !== null
      ? ` ${view.completedPools}/${view.targetPools}`
...
function StatusBar({ snapshot, lastSuccessfulRefresh, onSystem, onWallet }: { snapshot: DashboardSnapshot; lastSuccessfulRefresh: string | null; onSystem: () => void; onWallet: () => void }) {
  return <div className="terminal-statusbar">
    ...
    <span className="auto-refresh-status">REST对账：30秒 · WS实时</span>
...
function RankingRow({ pair, rank, expanded, onToggle, onEvidence, positionPoolIds, onCopied, showNet }: { pair: RankingPair; rank: number; expanded: boolean; onToggle: () => void; onEvidence: () => void; positionPoolIds: Set<string>; onCopied: (label: string) => void; showNet: boolean }) {
  ...
  const isRanked = pair.estimatedFeeIncome.value !== null || Boolean(pool && pool.lpFee24h !== null && pool.tvl !== null && pool.tvl > 0);
  ...
  return <>
    <tr className={`ranking-row ${expanded ? "expanded" : ""}`} onClick={onToggle} tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onToggle(); } }} data-testid={`pair-row-${rowId}`}>
      ...
      <td className="numeric">{stockPrice(pair.currentPrice ?? pool?.currentPrice ?? null)}</td><td className="numeric">{pool ? priceBand(pool.price24hLow, pool.price24hHigh) : <span className="metric-unavailable">24h区间暂无</span>}</td>
...
export default function DashboardClient({ initialSnapshot, initialRanking }: { initialSnapshot: DashboardSnapshot; initialRanking: RankingResponse }) {
  ...
  const rankingMatches = ranking?.capital === capital && ranking.window === window;
  const usingCached24hFallback = !rankingMatches && window !== "24h" && ranking?.capital === capital && ranking?.window === "24h";
  ...
  const pairs = rankingMatches || usingCached24hFallback ? ranking?.pairs ?? [] : [];
  const displayPairs = rankingMatches || usingCached24hFallback
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        const response = await fetch(`/api/rankings?capital=${capital}&window=${window}`, { cache: "no-store", headers: { "cache-control": "no-cache" } });
        ...
        setError(payload.window !== "24h" && payload.pairs.length === 0
  ...
  const rankingNote = ranking?.rankingBasis === "NET_PROFIT"
    ? "按预计净利润 USDC 降序"
  ...
  return <main className="terminal-shell">
    ...
    {usingCached24hFallback ? <div className="ranking-error" data-testid="window-fallback">当前窗口尚未完成可排名 Pool，先显示上次成功的24小时官方表；点击窗口按钮查看进度</div> : null}
    <section className="terminal-table-wrap"><div className="table-scroll"><table className="terminal-table public-ranking-table"><colgroup><col className="col-rank" /><col className="col-pair" /><col className="col-pool" /><col className="col-price" /><col className="col-range24h" /><col className="col-tvl" /><col className="col-volume" /><col className="col-fee" /><col className="col-estimate" /><col className="col-verdict" /></colgroup><thead><tr><th>排名</th><th>交易对</th><th>推荐 Pool / Fee Tier</th><th>股票价格</th><th>24小时上下限</th><th>TVL</th><th>{selectedStatus.label}成交量</th><th>{selectedStatus.label} LP Fee</th><th>预计手续费收入</th><th>结论 / 原因</th></tr></thead><tbody>{displayPairs.map((pair, index) => <RankingRow key={pair.pairId} pair={pair} rank={index + 1} expanded={expandedPair === pair.pairId} onToggle={() => setExpandedPair((value) => value === pair.pairId ? null : pair.pairId)} onEvidence={() => setEvidencePairId(pair.pairId)} positionPoolIds={positionPoolIds} onCopied={copied} showNet={showNet} />)}</tbody></table></div></section>
