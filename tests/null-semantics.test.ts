...
test("a complete zero-trade window remains a real zero", () => {
  ...
  const evidence = {
    ...
    persisted: true,
    source: "test",
    windowStart: "2026-08-06T11:00:00.000Z",
    windowEnd: asOf,
    startSlot: null,
    endSlot: null,
    ...
    gapCount: 0,
    oldestCoveredBlockTime: "2026-08-06T10:59:59.000Z",
  };
  const result = buildPersistedPoolMetrics({
    ...
    sourceCoverage: {
      "pool-zero": Object.fromEntries(WINDOW_KEYS.map((window) => [window, evidence])) as PoolSnapshot["windowCoverage"],
    },
    asOf,
  });
